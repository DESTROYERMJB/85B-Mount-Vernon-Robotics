...
// long-lived motion task. Reused for every motion instead of spawning a new task per call, to
// avoid the ~80KB stack allocation (and the latency that comes with it) that a fresh pros::Task
// would cost on every single motion over the course of an autonomous routine.
static pros::Task _motionTask([] {
    while (pros::Task::notify_take(true, NOTIFICATION_TIMEOUT)) {
        ...
        if (_motion.has_value()) {
            ...
            } catch (...) { _logHelper.error("motion threw an unknown exception"); }
        }
